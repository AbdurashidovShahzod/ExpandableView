...
import kotlinx.android.synthetic.main.child_item_todo.view.*
import kotlinx.android.synthetic.main.group_item.view.*
import kotlinx.android.synthetic.main.group_item_todo.view.*
import uz.unzosoft.expandableviewdemo.R

class MyExpandableTodoAdapter(var listTitle: List<String>, var map: HashMap<String, List<String>>) :
    ...
    override fun getGroupView(
        ...
        if (p2 == null) {
            groupItemView =
                LayoutInflater.from(p3?.context).inflate(R.layout.group_item_todo, p3, false)
        } else {
            groupItemView = p2
        }
        groupItemView.tvGroupTodo.text = listTitle[groupPosition]
    ...
    override fun getChildView(
        ...
        if (convertView == null) {
            childItemView = LayoutInflater.from(parent?.context)
                .inflate(R.layout.child_item_todo, parent, false)
        } else {
            childItemView = convertView
        }
        val list = map[listTitle[groupPosition]]
        val childPosition = list!![childPosition]
        childItemView.tvChildTodo.text = childPosition
